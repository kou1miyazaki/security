...
//-------------------------------------------------------------------
  ...
  //---------------------------------------------------------
    ...
    $URL = "http://security-miyazaki.azurewebsites.net/security/attention.html";  // リダイレクト先のURL
...
function sendmail($body){
  ...
  // 送信情報の設定
  //---------------------------------------------------------
    $host = "send.city.co.jp";    // SMTPサーバー名
    $port = 25;                  // ポート番号
    $auth = false;                 // SMTP認証を使用するか否か
    $username = "";               // SMTP認証のユーザ名
    $password = "";               // SMTP認証のパスワード
    $to = "miyazaki@city.co.jp";                     // 送信先アドレス
    $from = "miyazaki@city.co.jp";                   // 送信元アドレス
